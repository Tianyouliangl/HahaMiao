...
import me.hgj.jetpackmvvm.state.ResultState.Companion.onAppError
...
fun <T> MutableLiveData<ResultState<T>>.paresResult(result: BaseResponse<T>) {
    value = when {
        result.isSucces() -> {
            if (result.getResponseCode() == 700) onAppError(
                AppException(
            ...
            onAppError(AppException(result.getResponseCode(), result.getResponseMsg()))
        }
    }
}
...
fun <T> MutableLiveData<ResultState<T>>.paresException(e: Throwable) {
    this.value = onAppError(ExceptionHandle.handleException(e))
}
